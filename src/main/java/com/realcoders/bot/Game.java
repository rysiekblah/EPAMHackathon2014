...
public class Game {
	...
	public CardType move() {
		CardType card = ourPlayer.getCards().get(
				(int) (Math.random() * ourPlayer.getCards().size()));
	...
	public CardType retrieve() {
		CardType card = ourPlayer.getUsed().get(
				(int) (Math.random() * ourPlayer.getUsed().size()));
	...
	public Region condotiere() {
		...
		do {
			region = availableRegions
					.get((int) (Math.random() * availableRegions.size()));
	...
	public Region bishop() {
		...
		region = availableRegions.get((int) (Math.random() * availableRegions
				.size()));
